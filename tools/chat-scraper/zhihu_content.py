...
分工边界（2026-09-09 实测定稿）：
    ...
    - **通用阅读线拆分（v3.28，C1 裁决）**：SSRF 护栏/错误页识别/HTTP+
      浏览器双线原语拆至 read_page.py（零知乎依赖，与 MCP read_page 工具
      名对齐）；本模块保留知乎 API 线 + read() 分流器（知乎 API 分流 +
      bilibili 特化 + 外域委托 read_page.read_generic）。下述 ReadError/
      _check_http_url/CONTENT_LIMIT 等符号改为从 read_page 重导出，调用方
      与测试的引用路径不变。
...
import read_page as _read_page
import zhihu_sign
# v3.28 拆分兼容层：通用阅读线原语移居 read_page.py（零知乎依赖），
# 此处按原名重导出——调用方与测试的 zc.<name> 引用路径不变。
from read_page import (CONTENT_LIMIT, ReadError, _GENERIC_SELECTORS,     # noqa: F401
                       _HttpSoftFail, _check_http_url, _content_field,   # noqa: F401
                       _generic_read_http, _raise_if_error_page,         # noqa: F401
                       read_generic)                                     # noqa: F401
...
# 正文截断上限 CONTENT_LIMIT / _content_field 自 v3.28 起移居 read_page.py
# （模块头部重导出，引用路径不变）
...
# ---- 通用阅读线自 v3.28 起移居 read_page.py ------------------------------
# （SSRF 护栏/错误页判据/HTTP+浏览器双线/编排 read_generic；本模块经模块
#   头部重导出保持 zc.<name> 引用兼容，知乎分流与 bilibili 特化留在 read()）
