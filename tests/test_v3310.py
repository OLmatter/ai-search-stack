...
# ---------------------------------------------------------------------------
# 4. 版本锁 3.31.0 + 文档（自 test_v3300 接管精确锁）
# ---------------------------------------------------------------------------
class TestVersionSyncV331(unittest.TestCase):
    def test_versions_3310(self):
        # v3.31 起精确锁自 test_v3300 接管（v3.28→v3.29→v3.30 先例）：
        # 双 __version__（chat-scraper __init__ + mcp_server）同步钉死
        init_src = (REPO / "tools" / "chat-scraper" / "__init__.py"
                    ).read_text(encoding="utf-8")
        m = re.search(r'__version__ = "([^"]+)"', init_src)
        self.assertEqual(m.group(1), "3.31.0")
        self.assertIn("chat-scraper v3.31.0", init_src)
        try:
            import mcp_server              # noqa: F401
            self.assertEqual(mcp_server.__version__, "3.31.0")
        except ImportError:
            src = (REPO / "tools" / "mcp_server.py").read_text(
                encoding="utf-8")
            self.assertIn('__version__ = "3.31.0"', src)
    ...
    def test_changelog_and_readme_3310(self):
        ...
        self.assertIn("release-v3.31.0", readme)
        self.assertIn("v3.31.0（2026-09-17）", readme)
        # 测试徽章数随本批钉死（下一批交接时降常青）：
        # 517（v3.30 基线）+ 本批钉数（监控环 20 + 注册器 8 + server 4
        # + 版本锁/文档 3 = 35）= 552
        m = re.search(r"tests-(\d+)%20passing", readme)
        self.assertIsNotNone(m)
        self.assertEqual(m.group(1), "552")
